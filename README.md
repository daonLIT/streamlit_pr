...
이 프로젝트에서 저는 **Task 2(데이터 표시/지표)**, **Task 3(차트 시각화)**를 중심으로 구현했습니다.

- Task 2: 입력 데이터(`penguins.csv`)를 화면에 표시하고, 주요 통계값을 metric으로 요약해 한눈에 보이도록 구현
- Task 2: 평균 체중/부리 길이/부리 두께 등 핵심 지표를 정리해 데이터 해석 흐름을 개선
- Task 3: line, bar, area, scatter 차트를 적용해 같은 데이터를 다양한 관점으로 탐색할 수 있도록 구성
- Task 3: 종별 평균 체중 비교 및 변수 간 관계 확인이 가능하도록 시각화 로직 구현

### 배운 점
- Streamlit에서 데이터 로딩 → 요약 지표 → 차트 시각화로 이어지는 기본 분석 대시보드 흐름을 익혔습니다.
- 같은 데이터도 표현 방식(지표/차트)에 따라 전달력이 달라진다는 점을 실습으로 경험했습니다.
